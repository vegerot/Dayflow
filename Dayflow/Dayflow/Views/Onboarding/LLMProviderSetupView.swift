...
struct LLMProviderSetupView: View {
    ...
    private func saveConfiguration() {
        ...
        if activeProviderType == "doubao" {
            ...
            let resolvedBaseURL = baseURL.isEmpty ? "https://ark.cn-beijing.volces.com/api/v3" : baseURL
            ...
            UserDefaults.standard.set(resolvedBaseURL, forKey: "llmDoubaoBaseURL")
            UserDefaults.standard.set(resolvedModelId, forKey: "llmDoubaoModelId")
        }
...
class ProviderSetupState: ObservableObject {
    ...
    @Published var doubaoBaseURL: String = (UserDefaults.standard.string(forKey: "llmDoubaoBaseURL") ?? "https://ark.cn-beijing.volces.com/api/v3")
    @Published var doubaoModelId: String = (UserDefaults.standard.string(forKey: "llmDoubaoModelId") ?? "doubao-seed-1-6-flash-250828")
    ...
    private func persistDoubaoAPIKey(source: String) -> Bool {
        ...
        if stored {
            ...
            let base = doubaoBaseURL.trimmingCharacters(in: .whitespacesAndNewlines)
            let resolvedBase = base.isEmpty ? "https://ark.cn-beijing.volces.com/api/v3" : base
            let model = doubaoModelId.trimmingCharacters(in: .whitespacesAndNewlines)
            let resolvedModel = model.isEmpty ? "doubao-seed-1-6-flash-250828" : model

            UserDefaults.standard.set(resolvedBase, forKey: "llmDoubaoBaseURL")
            UserDefaults.standard.set(resolvedModel, forKey: "llmDoubaoModelId")
            UserDefaults.standard.set("doubao", forKey: "selectedLLMProvider")
